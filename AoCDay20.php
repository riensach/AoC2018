...
$lastStringPosition = count(str_split($directions,1));
$createdGrid[200][200] = 'X';
createRoom($createdGrid,1,$lastStringPosition);
echo "count - $lastStringPosition<br>";
function createRoom(&$createdGrid, $stringPosition, $lastStringPosition, $xPosition = 200, $yPosition = 200) {
    global $directions;    
    $continueIteration = 0;
    $startStringXPosition = $xPosition;
    $startStringYPosition = $yPosition;
    $startStringPosition = $stringPosition;
    while($continueIteration < 1 && $stringPosition <= $lastStringPosition) {
        $nextMove = substr($directions, $stringPosition, 1);
        if(preg_match('(\(|\)|\|)', $nextMove) === 1) {
            // The string contains one of ( or ) or |, we need to do something else
            if($nextMove == '(') {
                // It's a new optional set of routes, start a new line of query
                $endStringPosition = strpos($directions,')',$stringPosition);
                //echo "$nextMove - here - $stringPosition - X:$xPosition Y:$yPosition - X:$startStringXPosition Y:$startStringYPosition<br>";
                $stringPosition = createRoom($createdGrid,$stringPosition+1,$lastStringPosition,$xPosition,$yPosition);
               // echo "returned<br>";
            } elseif($nextMove == '|') {
                // We're already in an optional set of routes. Map this route out using the current location to the end location
                $endStringPosition = strpos($directions,')',$stringPosition);
               // echo "$nextMove - here2 - $stringPosition - X:$startStringXPosition Y:$startStringYPosition - X:$xPosition Y:$yPosition<br>";
                $xPosition = $startStringXPosition;
                $yPosition = $startStringYPosition;
                $stringPosition++;
                //$stringPosition = createRoom($createdGrid,$stringPosition+1,$lastStringPosition,$startStringXPosition,$startStringYPosition);
                //return $stringPosition;
                //echo "returned2<br>";
            } elseif($nextMove == ')') {
                // We're ending this routine. Finish up and return to the previous location
               // echo "$nextMove - got here3 - $stringPosition - X:$xPosition Y:$yPosition - X:$startStringXPosition Y:$startStringYPosition<br>";
                return $stringPosition+1;
            }
            
            
            //$continueIteration = 1; // For now end iteration to test            
           // echo "do something else<br>";
            ...
            // We should be free to move
            //echo "we can move $nextMove - X:$xPosition Y:$yPosition<br>";
            
            if($nextMove=='W') {
                if(!isset($createdGrid[$xPosition])) $createdGrid[$xPosition] = array();
                
                if(!isset($createdGrid[$xPosition-1])) $createdGrid[$xPosition] = array();
                if(!isset($createdGrid[$xPosition+1])) $createdGrid[$xPosition] = array();
                $createdGrid[$xPosition][$yPosition-1] = '|';
                //$createdGrid[$xPosition-1][$yPosition-1] = '#';
                //$createdGrid[$xPosition+1][$yPosition-1] = '#';
               $createdGrid[$xPosition][$yPosition-2] = '.';
                $xPosition = $xPosition;
               ...
               // break;
                ...
                if(!isset($createdGrid[$xPosition])) $createdGrid[$xPosition] = array();
                if(!isset($createdGrid[$xPosition-1])) $createdGrid[$xPosition] = array();
                if(!isset($createdGrid[$xPosition-2])) $createdGrid[$xPosition] = array();
                $createdGrid[$xPosition-1][$yPosition] = '-';
                //$createdGrid[$xPosition-1][$yPosition-1] = '#';
                //$createdGrid[$xPosition-1][$yPosition+1] = '#';
                $createdGrid[$xPosition-2][$yPosition] = '.';
...
$pathOptions = array();
$roomArray = array();
pathFinder($createdGrid,$roomArray);
function pathFinder(&$createdGrid, &$roomArray, $doorCount = 0, $xPosition = 200, $yPosition = 200, $xPositionParent = 0, $yPositionParent = 0) {
    global $pathOptions;
    $blocked = 0;    
    $lastX = $xPosition;
    $lastY = $yPosition;
    while($blocked < 1) {        
        $doorAbove = (($createdGrid[$xPosition-1][$yPosition] == '-' && !($xPosition-2==$xPositionParent && $yPosition==$yPositionParent) && !($xPosition-2==$lastX && $yPosition==$lastY)) ? 1:0);
        $doorBelow = (($createdGrid[$xPosition+1][$yPosition] == '-' && !($xPosition+2==$xPositionParent && $yPosition==$yPositionParent) && !($xPosition+2==$lastX && $yPosition==$lastY)) ? 1:0);
        $doorLeft = (($createdGrid[$xPosition][$yPosition-1] == '|' && !($xPosition==$xPositionParent && $yPosition-2==$yPositionParent) && !($xPosition==$lastX && $yPosition-2==$lastY)) ? 1:0);
        $doorRight = (($createdGrid[$xPosition][$yPosition+1] == '|' && !($xPosition==$xPositionParent && $yPosition+2==$yPositionParent) && !($xPosition==$lastX && $yPosition+2==$lastY)) ? 1:0);
        $totalDoorCount = $doorAbove + $doorBelow + $doorLeft + $doorRight;
        if(isset($roomArray[$xPosition."-".$yPosition])) {
           if($roomArray[$xPosition."-".$yPosition] > $doorCount) { 
               $roomArray[$xPosition."-".$yPosition] = $doorCount;
           }
        } else {
            $roomArray[$xPosition."-".$yPosition] = $doorCount;
        }
        if($doorAbove==0 && $doorBelow==0 && $doorLeft==0 && $doorRight==0) {
            //echo "ii";
            $blocked = 1;
            continue;
            ...
            // Multiple doors!
            if($doorAbove==1) {
                pathFinder($createdGrid,$roomArray,$doorCount+1,$xPosition-2,$yPosition,$xPosition,$yPosition);
            }
            if($doorBelow==1) {
                //echo "hi $doorCount";
                pathFinder($createdGrid,$roomArray,$doorCount+1,$xPosition+2,$yPosition,$xPosition,$yPosition);
            ...
            if($doorLeft==1) {                
                pathFinder($createdGrid,$roomArray,$doorCount+1,$xPosition,$yPosition-2,$xPosition,$yPosition);
            }
            if($doorRight==1) {
                pathFinder($createdGrid,$roomArray,$doorCount+1,$xPosition,$yPosition+2,$xPosition,$yPosition);
            }
...
array_multisort(array_column($pathOptions, 'totalDoors'), SORT_DESC,
                $pathOptions);



$totalCountOver1k = 0;
foreach($roomArray as $id => $value) {
...
echo "<br>total rooms with paths from start over 1k = $totalCountOver1k<br>";


$var = print_r($roomArray,true);
echo "<pre>$var</pre>"; 

$var = print_r($pathOptions,true);
echo "<pre>$var</pre>"; 
