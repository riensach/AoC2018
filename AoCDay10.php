...
$inputData = ("position=<-31138, -10302> velocity=< 3,  1>
...
$rawInputArrayRows = explode("\n",$inputData);
$dimensionsArray = array();
$bestDimensionsArray = array();
$boxGrid = array();

foreach ($rawInputArrayRows as $key => $value) {
    $initialDataExplode = explode(",",$value);
    $positionX = ltrim(substr($initialDataExplode[0],-6));
    $positionY = ltrim(substr($initialDataExplode[1],0,7));
    $velocityX = ltrim(substr($initialDataExplode[1],-2));
    $velocityY = ltrim(substr($initialDataExplode[2],0,3));
    
    
    
    $dimensionsArray[] = array('positionX' => $positionX, 'positionY' => $positionY, 'velocityX' => $velocityX, 'velocityY' => $velocityY);   
    
}
$var = print_r($dimensionsArray,true);
//echo "<pre>$var</pre>";   
...
while($c < 1000) {
    while ($r < 1000)  {
        ...
        if(!isset($boxGrid[$c])) $boxGrid[$c] = array();
        $boxGrid[$c][$r] = '.';
        $r++;     
    }
    $c++;
...
$bottomRightX = 500000;
$bottomRightY = 500000;  
$currentTotalDifference = (abs($topLeftX) + abs($topLeftY)) + (abs($bottomRightX) + abs($bottomRightY));


$c = 0;
$bestArrayIterationValue = 0;
while($c <= 12000) {
    $allPositive=1;
    foreach($dimensionsArray as $id => $dimensionValues) {
        
        $positionX = $dimensionValues['positionX'];
        $positionY = $dimensionValues['positionY'];
        $velocityX = $dimensionValues['velocityX'];
        $velocityY = $dimensionValues['velocityY'];
        
        $newPositionX = $dimensionValues['positionX'] + $dimensionValues['velocityX'];
        $newPositionY = $dimensionValues['positionY'] + $dimensionValues['velocityY'];
        $dimensionsArray[$id]['positionX'] = $newPositionX;
        $dimensionsArray[$id]['positionY'] = $newPositionY;
        if($newPositionX>=0 && $newPositionY>=0) {
            // After changing positions, at least some positions will still not be on the map, so lets not show anything.
            $allPositive=0;
        }
        //echo "Update $positionX - $positionY to $newPositionX - $newPositionY due to velocity updates of $velocityX and $velocityY <br>";
        
    }
    
   
    
    array_multisort(array_column($dimensionsArray, 'positionX'), SORT_ASC,
                array_column($dimensionsArray, 'positionY'),      SORT_ASC,
                $dimensionsArray);
    $foundTopLeftX = $dimensionsArray[0]['positionX'];
    $foundTopLeftY = $dimensionsArray[0]['positionY'];
    array_multisort(array_column($dimensionsArray, 'positionX'), SORT_DESC,
                array_column($dimensionsArray, 'positionY'),      SORT_DESC,
                $dimensionsArray);
    $foundBottomRightX = $dimensionsArray[0]['positionX'];
    $foundBottomRightY = $dimensionsArray[0]['positionY'];
    /* usort($dimensionsArray, function($a, $b) {
     ...
     * 
     */
    $totalDifference = (abs($foundTopLeftX) + abs($foundTopLeftY)) + (abs($foundBottomRightX) + abs($foundBottomRightY));
    //echo "$totalDifference - $currentTotalDifference<br>";
	if($totalDifference<$currentTotalDifference || ($c==10460 || $c==10461 || $c==10459 || $c==10458)) {
        // Found a better array!
        $currentTotalDifference = $totalDifference;
        $bestDimensionsArray = $dimensionsArray;
        $bestArrayIterationValue = $c;
		
       // echo "better array found - difference = $totalDifference<br>";
    }
	// Iteratino 10458 is right
	if($c==10460 || $c==10461 || $c==10459 || $c==10458) {
		echo "Iteration $c - $totalDifference vs $currentTotalDifference<br>";
		break;
	}
    ...
    /*if($allPositive==1){
        //All dimensions are positive, so lets so what they look like!
        echo "POPSITIVE<br>";
        displayCurrentGrid($dimensionsArray,$boxGrid);
    }
    if($allPositive==0 && ($c % 10000==0) && $c>0) {
       echo "NOT POSITIVE LOOP $c<br>";
        displayCurrentGrid($dimensionsArray,$boxGrid); 
    }
     * 
     */
...
echo "DISPLAYING THE BEST ARRAY - ITERATION $bestArrayIterationValue!<br>";

$var = print_r($bestDimensionsArray,true);
echo "<pre>$var</pre>";  

        displayCurrentGrid($bestDimensionsArray,$boxGrid);



function displayCurrentGrid($dimensionsArray,$boxGrid) {
    $clearedBoxGrid=refreshGrid($boxGrid); // Used to make sure the grid is back in its normal state
    $updatedBoxGrid=updateGrid($clearedBoxGrid,$dimensionsArray); // Update the grid to have hash marks where items are
...
function updateGrid($boxGrid,$dimensionsArray) {

    foreach($dimensionsArray as $id => $dimensionValues) {
        $positionX = $dimensionValues['positionX'];
        $positionY = $dimensionValues['positionY'];
        $boxGrid[$positionX][$positionY] = '#';
    }
    return $boxGrid;
}
...
foreach ($splitArray as $key => $value) {
   ...
   //$baseBoxArea[$value['leftEdge']][$value['topEdge']] = $baseBoxArea[$value['leftEdge']][$value['topEdge']]+1;
   $r=0;
   $c=0;
   $width = $value['width'];
   $height = $value['height'];
   //echo "$width - $height <br>";
    while($c < $value['width']) {
        $width = $value['leftEdge'] + $c;
        $height = $value['topEdge'];
        //echo "$width - $height ::: ";
        while ($r < $value['height'])  {            
            $height = $value['topEdge'] + $r;
            
            $baseBoxArea[$width][$height] = $baseBoxArea[$width][$height]+1;
			if(!isset($splitArray[$key]['hitLocation'])) $splitArray[$key]['hitLocation'] = array();
			$splitArray[$key]['hitLocation'][] = "$width,$height";
           // print_r($value);
           // echo "<br>$width - $height - ".$baseBoxArea[$width][$height]." ::<br>";
            $r++;
        }
        $c++;
        $r=0;
...
while($c < 1000) {
    while ($r < 1000)  {
        ...
        $areaCount = $baseBoxArea[$c][$r];
        if($areaCount > 1) {
            $overlapInchs = $overlapInchs + 1;
        } 
           $r++;     
    }
    $c++;
...
echo "Total overlap inchs: $overlapInchs";
...
foreach ($splitArray as $key => $value) {
	...
	foreach ($value['hitLocation'] as $key2 => $value2) {
		$hitLocation = explode(",",$value2);
		$width = $hitLocation[0];
		$height = $hitLocation[1];
		
		if($baseBoxArea[$width][$height]>1) {
			continue 2;
		}
	}
	echo "<br>Found it - it's ID #".$value['id']."<br>";
