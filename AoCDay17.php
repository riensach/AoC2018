...
$rowStartingValues = array();
$gridData = array();
$puzzleRows = explode("\n",$puzzleInput);

// REMEMBER TO CHANGE THESE FOR THE REAL DATA
...
foreach($puzzleRows as $puzzleID => $puzzleData) {
    $splitSections = explode(", ",$puzzleData);

    if(strpos($splitSections[0],'y')>-1) {
        $firstColumnStart = str_replace('y=','',$splitSections[0]);
        $firstColumn = 'y';
    } else {
        $firstColumnStart = str_replace('x=','',$splitSections[0]);
        $firstColumn = 'x';
    }
    if(strpos($splitSections[1],'y')>-1) {
        $yValue = str_replace('y=','',$splitSections[1]);
        $yExplode = explode("..",$yValue);
        $secondColumnStart = $yExplode[0];
        $secondColumnEnd = $yExplode[1];
    } else {
        $xValue = str_replace('x=','',$splitSections[1]);
        $xExplode = explode("..",$xValue);
        $secondColumnStart = $xExplode[0];
        $secondColumnEnd = $xExplode[1];
    }  
    
    if($firstColumn=='x') {        
        if($firstColumnStart>$longestX) $longestX = $firstColumnStart;
        if($firstColumnStart<$shortestX) $shortestX = $firstColumnStart;
        if($secondColumnEnd>$longestY) $longestY = $secondColumnEnd;
        if($secondColumnStart<$shortestY) $shortestY = $secondColumnStart;
    } else {        
        if($secondColumnEnd>$longestX) $longestX = $secondColumnEnd;
        if($secondColumnStart<$shortestX) $shortestX = $secondColumnStart;
        if($firstColumnStart>$longestY) $longestY = $firstColumnStart;
        if($firstColumnStart<$shortestY) $shortestY = $firstColumnStart;        
    }

    $rowStartingValues[] = array('firstColumn' => $firstColumn, 'firstColumnValue' => $firstColumnStart, 'secondColumnStart' => $secondColumnStart, 'secondColumnEnd' => $secondColumnEnd);
 
}
echo "<br>X is from $shortestX - $longestX and Y is from $shortestY - $longestY<br>";

for($c=0;$c<=$longestY+3;$c++) {
    for($r=$shortestX-1;$r<=$longestX+1;$r++) {
        if(!isset($gridData[$c])) $gridData[$c] = array();
        $gridData[$c][$r] = '.';
    }    
}
$gridData[0][500] = '+';
foreach($rowStartingValues as $rowID => $rowData) {
    ...
    if($rowData['firstColumn']=='y') { 
        for($r=$rowData['secondColumnStart'];$r<=$rowData['secondColumnEnd'];$r++) {
            $gridData[$rowData['firstColumnValue']][$r] = '#';
        }
    } else {
        for($r=$rowData['secondColumnStart'];$r<=$rowData['secondColumnEnd'];$r++) {
            $gridData[$r][$rowData['firstColumnValue']] = '#';
        }
    }
