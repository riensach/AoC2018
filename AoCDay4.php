...
$inputData = ("[1518-05-31 00:27] falls asleep
...
foreach ($arrayInfo as $key => $value) {
    ...
    $textValue = explode(" ",$value,3);
    $lookingForID = explode(" ",$textValue[2]);
    
    $date = $newValue[0];
    $time = $newValue[1];
    $datetime = $newValue[0] ." ". $newValue[1];
    $text = $textValue[2];
    
    if(substr( $lookingForID[1], 0, 1 ) === "#") {
        $guardID = substr( $lookingForID[1], 1);
    } else {
        $guardID = '';
    }
    ...
    if(strpos($text, 'begins shift') !== false) {
        $action = 'start';
    } elseif(strpos($text, 'wakes up') !== false) {
        $action = 'wake';
    } elseif(strpos($text, 'falls asleep') !== false) {
        $action = 'asleep';
    }
    
    
    
   // echo "$date :: $time :: $datetime :: Guard ID $guardID :: Action: $action :: $text<br>";
    
    
    $splitArray[] = array('id' => $guardID, 'datetime' => $datetime, 'date' => $date, 'time' => $time, 'action' => $action);
...
foreach($splitArray as $key => $value) {
    ...
    if(!isset($guardSleepMatrixID[$guardID])) {
        $guardSleepMatrixID[$guardID] = 0;
    }
    if($value['action']=='asleep') {
        $startSleepTime = substr( $value['time'], 3, 2 );
        $startSleepTime = ltrim($startSleepTime,'0');
        $startSleepDate = $value['date'];
    }
    if($value['action']=='wake') {
        $endSleepTime = substr( $value['time'], 3, 2 );
        $endSleepDate = $value['date'];
    }
    if($startSleepTime>0 && $endSleepTime > 0 && $guardID > 0 && $value['action']=='wake'){
        
        for($i=$startSleepTime;$i<$endSleepTime;$i++) {
            $asleepMatrix[] = array('id' => $guardID, 'date' => $startSleepDate, 'time' => $i);
            if($guardID==419) {
                //echo "guard $guardID asleep for $i on $startSleepDate - $endSleepDate - $startSleepTime - $endSleepTime - ".$value['time']."<Br>";
                //$totalMinutes++;
            }
        }
        $startSleepTime = 0;
...
//echo "TOTAL $totalMinutes";
$guardSleepMatrix = array();


foreach($guardSleepMatrixID as $key => $value) {
   for($t=0;$t<61;$t++) {
       $guardSleepMatrix[$key][$t] = 0;
   } 
}
    


foreach($asleepMatrix as $key => $value) {
    
    $guardID = $value['id'];
    //echo "$guardID - <Br>";
    $time = $value['time'];
    $guardSleepMatrix[$guardID][$time] = $guardSleepMatrix[$guardID][$time]+1;
    $guardSleepMatrixID[$guardID] = $guardSleepMatrixID[$guardID]+1;
}

arsort ($guardSleepMatrixID);



foreach($guardSleepMatrix as $key => $value) {
    
    arsort ($guardSleepMatrix[$key]);

}


$var = print_r($guardSleepMatrixID,true);
echo "<pre>$var</pre>";

$id = key($guardSleepMatrixID);
echo "-$id-";
arsort ($guardSleepMatrix[$id]);
$totalAsleep = 0;
foreach($guardSleepMatrix[$id] as $key => $value) {
    
    $totalAsleep = $totalAsleep + $value;

}
$answer = $id * $totalAsleep;
echo "Guard $id spent a total of $totalAsleep asleep - answer is $answer - 35,184 and part 2 is 37,886";
//usort($guardSleepMatrix[$id], function($a, $b) {
//  return $a[0] <=> $b[0];
//});



$var = print_r($guardSleepMatrix[$id],true);
echo "<pre>$var</pre>";
...
//array_multisort($guardSleepMatrix, SORT_DESC, SORT_NUMERIC, $array, $keys);


//array_multisort($guardSleepMatrixID, SORT_DESC, SORT_NUMERIC, $array, $keys);
//$array = array_combine($keys, $array);
$var = print_r($guardSleepMatrix,true);
echo "<pre>$var</pre>";